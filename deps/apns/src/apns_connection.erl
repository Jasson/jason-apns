...
init(Connection) ->
  error_logger:info_msg("~p:init ~p Connection=~p~n", [?MODULE, ?LINE, Connection]),
  try
    ...
    case ssl:connect(
      ...
      {ok, OutSocket} ->
        case ssl:connect(
               ...
               Connection#apns_connection.timeout) of
          {ok, InSocket} ->
               ?DEBUG("~p:int ~p OutSocket=~p, InSocket~p", [?MODULE, ?LINE, OutSocket, InSocket]),
...
handle_info({ssl, SslSocket, Data}, State = #state{in_socket  = SslSocket,
   ...
   ?DEBUG("~p:handle_info ssl in_socket ~p", [?MODULE, ?LINE]),
  case <<CurrentBuffer/binary, Data/binary>> of
...
handle_info({ssl_closed, SslSocket}, State = #state{in_socket = SslSocket,
   ...
   ?DEBUG("~p:int ~p SslSocket=~p", [?MODULE, ?LINE, SslSocket]),
...
handle_info(Request, State) ->
  ?DEBUG("~p:handel_info Requestion=~p", [?MODULE, ?LINE]), 
